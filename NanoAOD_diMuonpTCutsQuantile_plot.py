...
def diMuonPtCut(df, pT_diMuonCut):
    return df.Filter(f"diMuon_pT < {pT_diMuonCut}")
...
if plot == "yes":
    for var, label in variables.items():
        ...
        legend = ROOT.TLegend(0.12, 0.7, 0.42, 0.87)
        dummy = ROOT.TObject()
        ...
        for index, (ptcut, colour) in enumerate(zip(pt_cuts, colours)):
            ...
            h_SingleMuon = df_SingleMuon_pTCut.Histo1D(
                (f"h_SingleMuon_{var}_{ptcut}GeV", f"; {plot_xlabel}; Number of events (normalised)", 10, 0, 1),
                plot_column
            )
            ...
            # ratio.SetTitle(f"p^{{#mu#mu}}_{{T}} cut < {ptcut} GeV")
            ...
            legend.AddEntry(ratio, f"p^{{#mu#mu}}_{{T}} < {ptcut} GeV", "l")
            ratio_hists.append(ratio)

        if ratio_hists and plot_maximum > 0:
            # In ROOT, the first drawn histogram controls the pad axes.
            ratio_hists[0].SetMaximum(plot_maximum * 50)
            # ratio_hists[0].SetMinimum(1e-4)
            canvas.Modified()
            canvas.Update()

        legend.Draw()

        canvas.SaveAs(f"{var}_pTdiMuon<20GeV_Quantiletest.pdf")
        canvas.Close()